...
public class FontSelectorDialog extends Dialog<Font> {
	...
	public FontSelectorDialog(Font initialFont) {
		...
		initialFontFamily = getFontFamilyList().stream()
				.filter(ff -> ff.getFamilyName().equals(initialFont.getFamily())).findFirst().orElseGet(null);
		...
		setResultConverter(dialogButton -> dialogButton == ButtonType.OK
				? m_familyListView.getSelectionModel().getSelectedItem().getFont()
				: null);
