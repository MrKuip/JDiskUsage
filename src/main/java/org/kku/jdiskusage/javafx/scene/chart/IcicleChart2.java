  ...
  extends Region
{
  ...
  public IcicleChart2(Function<T, Double> valueSupplier)
  {
  ...
  private TreeItem<T> m_selectedTreeItem;
  private int m_firstVisibleLevel;

  private void createNodes(TreeItem<T> treeItem)
  {
    m_selectedTreeItem = treeItem;
    m_firstVisibleLevel = 0;
    createNodes(Arrays.asList(m_selectedTreeItem), null, m_firstVisibleLevel);
  }
  ...
  private void createNodes(List<TreeItem<T>> children, TreeItemRectangle parentRect, int level)
  {
    children.stream().map(ti -> getTreeItemValue(ti)).reduce(Double::sum).ifPresent(sum -> {
      ...
      children.stream().sorted(Comparator.comparing(IcicleChart2.this::getTreeItemValue, Comparator.reverseOrder()))
          .forEach(treeItem -> {
            ...
            if (percent > 0.005)
            {
              ...
              if (level == 0)
              {
                tir.heightProperty().bind(IcicleChart2.this.heightProperty().multiply(m_factorHeightProperty));
              ...
              tir.addEventHandler(MouseEvent.MOUSE_CLICKED, (me) -> {
                double fromFactorHeight;
                double toFactorHeight;
                double fromDeltaY;
                double toDeltaY;

                System.out.println("icicly chart height=" + IcicleChart2.this.getHeight());
                System.out.println("rect height=" + tir.getHeight());
                System.out.println("rect y=" + tir.getY());
                System.out.println("multiplier =" + (IcicleChart2.this.getHeight() / tir.getHeight()));
                ...
                else
                {
                  ...
                  currentFactor = IcicleChart2.this.getHeight() / tir.getHeight();
                ...
                timeline.setOnFinished(ae -> {
                  IcicleChart2.this.getChildren().forEach(node -> {
                    ...
                    IcicleChart2.this.getParent().getParent().requestLayout();
                  });
                });
              });
  ...
  private NumberBinding calculateRadixProperty(int level)
  {
    return m_radixPropertyMap.computeIfAbsent(level,
        key -> Bindings.min(IcicleChart2.this.widthProperty(), IcicleChart2.this.heightProperty())
